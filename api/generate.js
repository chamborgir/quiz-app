const MODEL_CONFIGS = {
  'gemini-3.5-flash-lite': { maxOutputTokens: 16000, label: 'Gemini 3.5 Flash-Lite (fastest, cheapest)' },
  'gemini-3.1-flash-lite': { maxOutputTokens: 16000, label: 'Gemini 3.1 Flash-Lite (frontier-class, low cost)' },
...
function isValidItem(mode, item, sourceMode) {
  if (mode === 'flashcard') {
    ...
    if (sourceMode === 'ai') return !isSelfReferential(item.front) && !isSelfReferential(item.back);
    return true;
  }
  if (!item.question || !Array.isArray(item.choices) || item.choices.length !== 4) return false;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (sourceMode === 'ai' && needsExpansion(sourceText) && timeLeft() > 15000) {
      try {
        ...
        console.warn('Expansion pass failed, continuing with original source:', err.message);
      }
    }
    ...
    function recentQuestionSummaries(limit) {
      ...
      return deduped
        .slice(-lim)
        .map(function (q) { return mode === 'flashcard' ? q.front : q.question; })
        .join(' | ') || 'none yet';
    }
      ...
      for (let i = 0; i < sizes.length; i += MAX_CONCURRENT) {
        ...
        const chunkPromises = chunk.map((size) => {
          ...
          const prompt = sourceMode === 'extract'
            ...
            : buildPrompt(mode, size, chunkText, batchIndex, avoidList, usedExpansion);
          return callGeminiWithRetry(apiKey, prompt).catch((err) => {
            console.error('Batch ' + batchIndex + ' failed permanently:', err.message);
            return [];
          });
        });
    ...
    if (mode === 'mcq' && timeLeft() > 12000) {
      finalQuestions = await verifyAnswers(apiKey, finalQuestions, sourceText, timeLeft);
    }
    ...
    res.status(500).json({
      ...
      debug: { stack: err.stack ? err.stack.slice(0, 800) : null, name: err.name },
    });
  }
}
...
async function verifyAnswers(apiKey, questions, sourceText, timeLeft) {
  ...
  for (let i = 0; i < chunks.length; i += VERIFY_MAX_CONCURRENT) {
    if (timeLeft() < 6000) {
      for (let j = i; j < chunks.length; j++) verifiedChunks[j] = chunks[j];
      break;
    }
    const group = chunks.slice(i, i + VERIFY_MAX_CONCURRENT);
    const groupPromises = group.map((chunk, j) => {
      const chunkIdx = i + j;
      return callGeminiWithRetry(apiKey, buildVerifyPrompt(chunk, sourceText))
        .then((result) => {
          verifiedChunks[chunkIdx] = (Array.isArray(result) && result.length === chunk.length) ? result : chunk;
        })
        .catch(() => {
...
function buildExpansionPrompt(sourceText) {
  return 'The text below is a short, dense list — likely law/act citations, acronym mappings (e.g. "RA 1234 -> Some Act Name"), memo/order numbers, or bare facts. On its own it is too brief to write many distinct, non-repetitive quiz questions from.\n\n' +
...
function buildPrompt(mode, count, sourceText, batchIndex, avoidList, usedExpansion) {
  const languageInstruction = 'IMPORTANT LANGUAGE RULE: The source material below may mix languages (e.g. Filipino/Tagalog sentences next to English ones). For EACH question, write it in the same language as the specific sentence/paragraph it is based on.';

  const formattingInstruction = 'FORMATTING RULE: If a question includes enumerated statements (e.g. "I. ... II. ..."), insert a literal newline (\\n) before each item.';

  const mathInstruction = 'MATH FORMATTING RULE: Write ALL math using LaTeX wrapped in single dollar signs — every symbol. Examples: $x^2$, $\\frac{a}{b}$, $\\infty$, $\\pi$.';

  const selfContainedInstruction = 'SELF-CONTAINED RULE — CRITICAL: Never write a question that references the source\'s own numbering/labeling (e.g. "problem 7," "page 2," "choice (C)"). Every question must be fully self-contained.';

  const noDiagramInstruction = 'TEXT-ONLY RULE: No images/diagrams are available. Never reference "the diagram," "the graph," or "the figure."';
...
function safeParseJsonArray(rawText) {
  ...
  throw new Error('Could not parse Gemini response as valid JSON, even after repair.');
}
